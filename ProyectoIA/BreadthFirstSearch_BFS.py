...
class BFS:
    ...
    def __init__(self, laberinto, inicio, meta):
        self.laberinto = np.array(laberinto)  # Usar una matriz numpy para acceso más rápido
        self.inicio = inicio
        self.meta = meta
        self.n, self.m = self.laberinto.shape
        self.ruta_minima = []
        self.queue = deque([(self.inicio, [])])
        self.visitado = np.zeros_like(self.laberinto, dtype=bool)  # Usar matriz numpy de booleanos
        self.visitado[self.meta[0], self.meta[1]] = False  # Asegurarse de que la celda de meta no esté marcada como visitada

    def bfs_laberinto(self):
        while self.queue:  # Mientras la cola no esté vacía
            (x, y), ruta_minima = self.queue.popleft()  # Obtener el primer elemento de la cola

            if (x, y) == self.meta:  # Si la celda actual es la meta, retornar la ruta actual
                ruta_minima.insert(0, self.inicio)  # Agregar la coordenada de inicio al principio de la ruta
                return ruta_minima

            for i in range(4):
                new_x = x + self.dx[i]  # Obtener las nuevas coordenadas de la celda a la que se puede mover
                new_y = y + self.dy[i]  # Obtener las nuevas coordenadas de la celda a la que se puede mover

                if (
                    0 <= new_x < self.n 
                    and 0 <= new_y < self.m # Si las nuevas coordenadas están dentro de los límites
                    and self.laberinto[new_x][new_y] != 1  # Para considerar la meta con valor 3, y no considerar los obstáculos con valor 1
                    and not self.visitado[new_x][new_y] # Si la celda no ha sido visitada
                ):
                    self.visitado[new_x][new_y] = True  # Marcar la celda como visitada
                    nueva_ruta = ruta_minima + [(new_x, new_y)]  # Agregar la celda a la ruta actual
                    self.queue.append(((new_x, new_y), nueva_ruta))  # Agregar la celda a la cola con la ruta actual
